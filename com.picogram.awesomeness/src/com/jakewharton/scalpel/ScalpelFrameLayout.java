...
public class ScalpelFrameLayout extends FrameLayout {
	...
	/**
	 * Set whether the view layers draw their contents. When false, only
	 * wireframes are shown.
	 */
	...
	public boolean onTouchEvent(
			@SuppressWarnings("NullableProblems") MotionEvent event) {
		if (!enabled) {
		...
		case MotionEvent.ACTION_POINTER_DOWN: {
			...
			if (pointerOne == INVALID_POINTER_ID) {
				...
				if (DEBUG)
					log("Got pointer 1!  id: %s  x: %s  y: %s", pointerOne,
							lastOneY, lastOneY);
				...
				if (DEBUG)
					log("Got pointer 2!  id: %s  x: %s  y: %s", pointerTwo,
							lastTwoY, lastTwoY);
			} else {
				if (DEBUG)
					log("Ignoring additional pointer.  id: %s",
			...
			if (pointerTwo == INVALID_POINTER_ID) {
				...
				for (int i = 0, count = event.getPointerCount(); i < count; i++) {
					if (pointerOne == event.getPointerId(i)) {
						...
						rotationY = Math.min(
								Math.max(rotationY + drx, ROTATION_MIN),
								ROTATION_MAX);
						rotationX = Math.min(
								Math.max(rotationX + dry, ROTATION_MIN),
								ROTATION_MAX);
						if (DEBUG) {
				...
				// We know there's two pointers and we only care about
				...
				if (multiTouchTracking == TRACKING_VERTICALLY) {
					...
					if (xOne >= xTwo) {
						spacing += (dxOne / getWidth() * SPACING_MAX)
								- (dxTwo / getWidth() * SPACING_MAX);
					} else {
						spacing += (dxTwo / getWidth() * SPACING_MAX)
								- (dxOne / getWidth() * SPACING_MAX);
					}

					spacing = Math.min(Math.max(spacing, SPACING_MIN),
							SPACING_MAX);
					invalidate();
				}
		...
		case MotionEvent.ACTION_POINTER_UP: {
			int index = (action != ACTION_POINTER_UP) ? 0 : event
					.getActionIndex();
		...
		while (!layeredViewQueue.isEmpty()) {
			...
			if (view instanceof ViewGroup) {
				...
				if (count > visibilities.length) {
					...
					if (DEBUG)
						log("Grow visibilities array to %s.", count);
				}
			...
			if (drawIds) {
				int id = view.getId();
				if (id != NO_ID) {
					canvas.drawText(nameForId(id), textOffset, textSize,
							viewBorderPaint);
				}
			}
			...
			// Restore any children and queue any visible ones for later
			// drawing.
			if (view instanceof ViewGroup) {
				ViewGroup viewGroup = (ViewGroup) view;
