
package com.picogram.awesomeness;

import android.app.Activity;
import android.app.AlertDialog;
import android.app.Dialog;
import android.content.DialogInterface;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager.NameNotFoundException;
import android.content.res.Resources;
import android.content.res.XmlResourceParser;
import android.util.Log;
import android.webkit.WebView;
import android.widget.Toast;

import org.xmlpull.v1.XmlPullParser;
import org.xmlpull.v1.XmlPullParserException;

import java.io.IOException;

/*
 * Class to show a changelog dialog
 * (c) 2012 Martin van Zuilekom (http://martin.cubeactive.com)
 *  
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 */

public class ChangeLogDialog {
    static final private String TAG = "ChangeLogDialog";

    static final private String TITLE_CHANGELOG = "title_changelog";
    static final private String CHANGELOG_XML = "changelog";

    private final Activity fActivity;

    public ChangeLogDialog(final Activity context) {
        this.fActivity = context;
    }

    // Get the current app version
    private String GetAppVersion() {
        try {
            final PackageInfo _info = this.fActivity.getPackageManager().getPackageInfo(
                    this.fActivity.getPackageName(), 0);
            return _info.versionName;
        } catch (final NameNotFoundException e) {
            e.printStackTrace();
            return "";
        }
    }

    // Get the changelog in html code, this will be shown in the dialog's
    // webview
    private String GetHTMLChangelog(final int aResourceId, final Resources aResource) {
        String _Result = "<html><head>" + this.GetStyle() + "</head><body>";
        final XmlResourceParser _xml = aResource.getXml(aResourceId);
        try
        {
            int eventType = _xml.getEventType();
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if ((eventType == XmlPullParser.START_TAG) && (_xml.getName().equals("release"))) {
                    _Result = _Result + this.ParseReleaseTag(_xml);

                }
                eventType = _xml.next();
            }
        } catch (final XmlPullParserException e)
        {
            Log.e(TAG, e.getMessage(), e);
        } catch (final IOException e)
        {
            Log.e(TAG, e.getMessage(), e);

        } finally
        {
            _xml.close();
        }
        _Result = _Result + "</body></html>";
        return _Result;
    }

    // CSS style for the html
    private String GetStyle() {
        return
        "<style type=\"text/css\">"
                + "h1 { margin-left: 0px; font-size: 12pt; }"
                + "li { margin-left: 0px; font-size: 9pt;}"
                + "ul { padding-left: 30px;}"
                + "</style>";
    }

    // Parse a the release tag and return html code
    private String ParseReleaseTag(final XmlResourceParser aXml) throws XmlPullParserException,
            IOException {
        String _Result = "<h1>Release: " + aXml.getAttributeValue(null, "version") + "</h1><ul>";
        int eventType = aXml.getEventType();
        while ((eventType != XmlPullParser.END_TAG) || (aXml.getName().equals("change"))) {
            if ((eventType == XmlPullParser.START_TAG) && (aXml.getName().equals("change"))) {
                eventType = aXml.next();
                _Result = _Result + "<li>" + aXml.getText() + "</li>";
            }
            eventType = aXml.next();
        }
        _Result = _Result + "</ul>";
        return _Result;
    }

    // Call to show the changelog dialog
    public void show() {
        // Get resources
        final String _PackageName = this.fActivity.getPackageName();
        Resources _Resource;
        try {
            _Resource = this.fActivity.getPackageManager().getResourcesForApplication(_PackageName);
        } catch (final NameNotFoundException e) {
            e.printStackTrace();
            return;
        }

        // Get dialog title
        String _Title = "Changelog";
        _Title = _Title + " v" + this.GetAppVersion();

        // Get Changelog xml resource id
        final int _resID = _Resource.getIdentifier(CHANGELOG_XML, "xml", _PackageName);
        // Create html change log
        final String _HTML = this.GetHTMLChangelog(_resID, _Resource);

        // Get button strings
        final String _Close = "Close";

        // Check for empty changelog
        if (_HTML.equals("") == true)
        {
            // Could not load change log, message user and exit void
            Toast.makeText(this.fActivity, "Could not load change log", Toast.LENGTH_SHORT).show();
            return;
        }

        // Create webview and load html
        final WebView _WebView = new WebView(this.fActivity);
        _WebView.loadData(_HTML, "text/html", "utf-8");
        final AlertDialog.Builder builder = new AlertDialog.Builder(this.fActivity)
                .setTitle(_Title)
                .setView(_WebView)
                .setPositiveButton(_Close, new Dialog.OnClickListener() {
                    public void onClick(final DialogInterface dialogInterface, final int i) {
                        dialogInterface.dismiss();
                    }
                });
        builder.create().show();
    }

}
